...
def _parse_akumulasi(resp):
    ...
    if isinstance(resp, dict):
        ...
        if isinstance(d.get("columns"), list) and isinstance(d.get("rows"), list):
            _try_table(d.get("columns"), d.get("rows"))
...
def main():
    ...
    resp = meta.get("data")
    _save_json(raw_json, meta)  # simpan RAW untuk debugging
    ...
    if not rows:
        ...
        # tetap update rolling agar file ada
    ...
    # 4) Rolling 5D & filter kategori hari-ini (3BA/2BA)
